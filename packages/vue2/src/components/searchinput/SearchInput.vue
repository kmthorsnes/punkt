    ...
    >
      ...
      <div :class="appearance === 'local' ? 'pkt-input__container' : 'pkt-searchinput__field'">
        <input
          ...
          type="search"
          v-bind="$attrs"
          v-on="{ ...$listeners, input: (event) => $emit('onSearch', event.target.value) }"
          :name="name || id"
          :id="id"
          :placeholder="placeholder"
          :value="value"
          :disabled="disabled"
          @keypress.enter.prevent="$emit('onSearch', value)"
          autocomplete="off"
        ...
        <PktButton
          ...
          :disabled="disabled"
          @click.prevent="$emit('onSearch', value)"
          @keypress.enter.prevent="$emit('onSearch', value)"
